...
impl Context {
    pub fn resize(&mut self, (width, height): (i32, i32)) {
        let bmpSize = std::mem::size_of::<BITMAPINFOHEADER>() + ((width * height) as usize) * std::mem::size_of::<RGBQUAD>();
        self.bmp_info = Some(vec![0 as u8; bmpSize]);
        let bmpInfo = unsafe { &mut *(self.bmp_info.as_mut().unwrap().as_mut_ptr() as *mut BITMAPINFO) };
        unsafe {
...
impl<E> crate::platform::common::PlatformContext<E> for Context {
    fn new(window: &mut Window, event_loop: &mut EventLoop<E>, window_widget: WidgetRef<dyn crate::widgets::Window>) -> Self {
            ...
            *control_flow = ControlFlow::Wait;
            ...
            match event {
                ...
                Event::RedrawRequested(_) => {
                    let bmp_info = unsafe { &mut *(self.bmp_info.as_mut().unwrap().as_mut_ptr() as *mut BITMAPINFO) };

                    let pixels_ptr = bmp_info.bmiColors.as_mut_ptr() as *const c_void;
                    let pixels = unsafe {
                        let mut ptr = pixels_ptr as *mut u8;
                        std::slice::from_raw_parts_mut(ptr, (self.size.0 * self.size.1) as usize * std::mem::size_of::<RGBQUAD>())
                    };
                    ...
                    unsafe {
                        ...
                        windows::Win32::Graphics::Gdi::StretchDIBits(hdc, 0, 0, self.size.0, self.size.1, 0, 0, self.size.0, self.size.1, pixels_ptr, bmp_info, DIB_RGB_COLORS, SRCCOPY);
                        windows::Win32::Graphics::Gdi::EndPaint(self.hwnd, &paint_struct);
                    }

                    unsafe { windows::Win32::Graphics::Gdi::InvalidateRect(self.hwnd, std::ptr::null(), false); }
                },
                Event::WindowEvent {
