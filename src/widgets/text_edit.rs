use cgmath::Vector2;
use skia_safe::{Color, Font, Paint, Point, Rect, scalar};
...
impl Widget for TextEditWidget {
    ...
    fn get_desired_size(&self) -> Vector2<scalar> {
        ...
        let text_size = self.foreground_font.measure_str(text, Some(&self.foreground));
    ...
    fn paint(&self, geometry: Geometry, layer: i32, painter: &mut Painter) -> i32 {
        let width = if self.text.len() >= self.cursor && self.cursor > 0 {
            self.foreground_font.measure_str(&self.text[0..self.cursor], Some(&self.foreground))
        } else {
            (0.0, Rect::default())
        };
        let Vector2{x, y} = geometry.absolute_pos();
        painter.canvas().draw_str(&self.text, Point::new(x + 10.0, y + geometry.local_size().y/2.0), &self.foreground_font, &self.foreground);
        painter.canvas().draw_line(Point::new(x + width.0 + 10.0, y + 0.0), Point::new(x + width.0 + 10.0, y + geometry.local_size().y), &self.foreground);
        layer+1
    }
